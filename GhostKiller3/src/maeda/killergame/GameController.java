...
public class GameController extends  SurfaceView implements SurfaceHolder.Callback {
	...
	public GameController(Context context) {
		...
		Stuff.menubackground1 = getResizedBitmap(Stuff.menubackground1,  height, width);		
		Stuff.menubackground2= getResizedBitmap(Stuff.menubackground2,  height, width);		
		
		reddie = new ReddieManager(context);
		enemy = new EnemyManager(context, reddie);
		star = new StarManager(width, height, context);
	...
	public void render(Canvas canvas) {
		...
		if(score<3000)
			...
			canvas.drawColor(Color.BLACK);
			canvas.drawBitmap(Stuff.menubackground1, 0, 0, null);
		}
		...
		int lives = 0;
		if(!reddie.reddies.isEmpty()){
			lives = reddie.reddies.get(0).lives;
		}
		canvas.drawText("Score: "+score+"               "+"Lives:  "+lives, /*Stuff.scoreingame.getWidth(), 15*/15,15, paint);
		
		if(gameState==GameStates.READY){
			//display message to tap
			canvas.drawText("Protect your soul by tapping on the blue ghosts!", 50, 50, paint);
			//canvas.drawText("to protect your red soul.", 50, 70, paint);
	...
	public States handleTouches(MotionEvent evt) {
		...
		if(gameState==GameStates.READY && now-changeState>1000)	{
			...
			int incscore = enemy.killEnemy(evt.getX(), evt.getY());
			//display combo awards
			reddie.setCombo(incscore);
			if(incscore>=20 && incscore<=30){
				incscore = incscore *2;
			}else if(incscore>=40 && incscore<50){
				incscore = incscore *3;
			}else if(incscore>=50 && incscore<60){
				incscore = incscore *4;
			}else if(incscore>=60) {
				incscore = incscore *5;
			}
			...
			if(!reddie.reddies.isEmpty()){
				reddie.reddies.get(0).lives = reddie.reddies.get(0).lives+ star.collide(evt.getX(), evt.getY());
			}
	...
	public void collision(){
		for(int i=0; i<enemy.enemies.size(); i++){
			for(int k=0; k<reddie.reddies.size(); k++){
				try{
					if(!enemy.enemies.isEmpty() && i<enemy.enemies.size()){
					...
					System.out.println("Indexoutofbounds exception found in collision method of Gamecontroller");
					break;
				}
			}
		}
